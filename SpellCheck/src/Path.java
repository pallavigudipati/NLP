    ...
    public List<List<String>> extractAllEdits(String source, String target) {
        ...
        for (int i = 1; i < pathCoords.size(); ++i) {
            ...
            if (prevPoint.x == currPoint.x) {
                wrong += "" + xTarget.charAt(prevPoint.y)
                        + xTarget.charAt(currPoint.y);
                ...
                type += "D";
            } else if (prevPoint.x + 1 == currPoint.x
                    && prevPoint.y + 1 == currPoint.y) {
                // Substitution
                wrong += "" + xTarget.charAt(currPoint.y);
                correct += "" + xSource.charAt(currPoint.x);
                type += "S";
            } else if (prevPoint.x + 2 == currPoint.x
                    && prevPoint.y + 2 == currPoint.y) {
                // Reversal
                wrong += "" + xTarget.charAt(prevPoint.y + 1)
                        + xTarget.charAt(currPoint.y);
                correct += "" + xSource.charAt(prevPoint.x + 1)
                        + xSource.charAt(currPoint.x);
                type += "R";
            }
            allEdits.add(new ArrayList<String>(Arrays.asList(type, wrong + "|"
                    + correct)));
            prevPoint = currPoint;
        }
        return allEdits;
