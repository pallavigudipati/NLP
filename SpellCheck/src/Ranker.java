...
*/
     ...
     * BufferedReader(new FileReader(filename)); String line = null; while
     * ((line = reader.readLine()) != null) { String[] parts = line.split(",");
     * unigramCounts.put(parts[0], Integer.parseInt(parts[1])); } } catch
     * (Exception e) { System.out.println("Not able to read unigrams"); } }
     * 
     * public void loadBigrams(String filename) { bigramCounts = new
     * HashMap<String, Integer>(); try { BufferedReader reader = new
     * BufferedReader(new FileReader(filename)); String line = null; while
     * ((line = reader.readLine()) != null) { String[] parts = line.split(",");
     * bigramCounts.put(parts[0], Integer.parseInt(parts[1])); } } catch
     * (Exception e) { System.out.println("Not able to read bigrams"); } }
     * 
     * public void parse(String filename) { priors = new HashMap<String,
     * Integer>(); try { BufferedReader reader = new BufferedReader(new
     * FileReader("count_1edit.txt")); String line = null; while ((line =
     * reader.readLine()) != null) { String[] parts = line.split("|"); if
     * (parts.length >= 2) { String wrong = parts[0]; } } } catch (Exception e)
